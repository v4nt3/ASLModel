...
def split_dataset(videos):
    ...
    # Mezclar aleatoriamente para evitar sesgo
    random.shuffle(videos)

    # Separar clases de forma estratificada
    labels = [v["label"] for v in videos]

    train_videos, temp_videos = train_test_split(videos, test_size=(1 - TRAIN_SPLIT), stratify=labels)
    val_videos, test_videos = train_test_split(temp_videos, test_size=TEST_SPLIT / (TEST_SPLIT + VAL_SPLIT),
